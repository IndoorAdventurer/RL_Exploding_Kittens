...
class EKAtomicReplayBuffer:
    ...
    def random_batch(self, batch_size: int):
        ...
        return (
            ...
            torch.tensor(np.array([[x[2]] for x in sample]), dtype=torch.int64, device="cuda"),
            ...
            torch.tensor(np.array([x[10] for x in sample], dtype=np.int32), dtype=torch.int32, device="cuda")
